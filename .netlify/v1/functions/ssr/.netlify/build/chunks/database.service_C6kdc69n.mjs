...
class DatabaseService {
  ...
  async healthCheck() {
    try {
      const { error } = await this.supabase.from("leads").select("id").limit(1);
      return !error;
    } catch (error) {
  ...
  async createAppointment(appointmentData) {
    try {
      ...
      const { data, error } = await this.supabase.from("appointments").insert(dbAppointment).select().single();
      if (error) {
   ...
   */
  async createOrFindLead(leadData) {
    try {
      console.log("[DatabaseService] Creating or finding lead for email:", leadData.email);
      const { data: existingLead, error: searchError } = await this.supabase.from("leads").select("id").eq("email", leadData.email).single();
      ...
      const newLead = {
        ...
        lead_status: "new",
        lead_score: 50,
        // Default score for appointment bookings
        highlevel_contact_id: leadData.highlevelContactId,
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
        updated_at: (/* @__PURE__ */ new Date()).toISOString(),
        last_activity_at: (/* @__PURE__ */ new Date()).toISOString()
      };
      const { data: createdLead, error: createError } = await this.supabase.from("leads").insert(newLead).select("id").single();
      if (createError) {
  ...
  async getAppointment(appointmentId) {
    try {
      ...
      const { data, error } = await this.supabase.from("appointments").select("*").eq("id", appointmentId).single();
      if (error) {
   ...
   */
  async incrementPollingAttempts(appointmentId) {
    try {
      ...
      const { data, error } = await this.supabase.from("appointments").update({
  ...
  subscribeToAppointment(appointmentId, callback) {
    try {
      ...
      const subscription = this.supabase.channel(`appointment-${appointmentId}`).on(
        "postgres_changes",
        {
          event: "UPDATE",
          schema: "public",
